...
(function () {
  ...
  const mount = (form) => {
    ...
    [name, qty, unit].forEach(el => el.addEventListener('input', sync));
    ...
    form.addEventListener('submit', () => {
      ...
      const meta = { name: partName, quantity, unitCost, total: quantity * unitCost, submittedAt: Date.now() };
  ...
  const injectStyle = () => {
    ...
    style.textContent = `
      ...
      .gf-pp-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
      .gf-pp-field label{display:block;font-size:11px;font-weight:800;color:#94a3b8;margin-bottom:5px}
      .gf-pp-field input{width:100%;box-sizing:border-box;padding:11px 12px;border-radius:12px;border:1px solid rgba(100,116,139,.55);background:#0f172a;color:#f8fafc;outline:none}
      .gf-pp-field input:focus{border-color:#3b82f6;box-shadow:0 0 0 2px rgba(59,130,246,.12)}

 ...
 * Adds explicit View / Edit / Delete / Print actions to customer-history cards.
 * Works against the existing localStorage data model and does not alter core React logic.
 */
(function () {
  ...
  function injectStyles() {
    ...
    s.textContent = `
      .gf-history-actions{display:flex!important;flex-wrap:wrap!important;gap:7px!important;margin-top:12px!important;padding-top:10px!important;border-top:1px solid rgba(100,116,139,.22)!important}
      .gf-history-action{display:inline-flex!important;align-items:center!important;justify-content:center!important;gap:5px!important;border:1px solid rgba(100,116,139,.45)!important;background:rgba(30,41,59,.72)!important;color:#e2e8f0!important;border-radius:10px!important;padding:7px 10px!important;font-size:11px!important;font-weight:900!important;cursor:pointer!important;min-height:36px!important;white-space:nowrap!important}
      .gf-history-action:hover{filter:brightness(1.18)!important}.gf-history-danger{color:#fda4af!important;border-color:rgba(244,63,94,.4)!important;background:rgba(127,29,29,.25)!important}.gf-history-print{color:#93c5fd!important}
      ...
      #gf-history-view-modal .gf-hv-box{width:min(620px,100%);margin:4vh auto;background:#0f172a;color:#e2e8f0;border:1px solid #334155;border-radius:22px;padding:18px;box-shadow:0 30px 90px rgba(0,0,0,.55)}
      .gf-hv-head{display:flex;justify-content:space-between;align-items:center;gap:10px}.gf-hv-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:14px}.gf-hv-item{padding:10px 12px;border-radius:12px;background:#111827;border:1px solid #263244}.gf-hv-label{display:block;font-size:10px;color:#94a3b8;font-weight:800;text-transform:uppercase}.gf-hv-value{display:block;margin-top:3px;font-size:13px;font-weight:800;overflow-wrap:anywhere}.gf-hv-full{grid-column:1/-1}
      @media(max-width:560px){.gf-history-action{flex:1 1 calc(50% - 7px)}.gf-hv-grid{grid-template-columns:1fr}.gf-hv-full{grid-column:auto}}
  ...
  function locate(id) {
    for (const type of ['repairs','devices']) {
      const arr=read(STORE[type]); const index=arr.findIndex(r=>String(r.id||'')===String(id));
  ...
  function showView(info) {
    ...
    const fields=[
      ['ID',r.id],['Customer / Party',r.customerName||r.partyName||'—'],['Phone',r.phone||r.partyPhone||'—'],
      ['Device / Model',r.model||r.brandModel||r.deviceType||'—'],['Status',r.status||'—'],
      ['Total',r.totalCost!=null?`NPR ${money(r.totalCost)}`:r.sellPrice!=null?`NPR ${money(r.sellPrice)}`:'—'],
      ['Paid',r.paidAmount!=null?`NPR ${money(r.paidAmount)}`:'—'],['Due',r.dueAmount!=null?`NPR ${money(r.dueAmount)}`:'—'],
      ['Warranty',r.warrantyMonths||'—'],['Date',r.dateTime||r.date||'—'],
      ['Issue / Details',r.issue||r.condition||r.notes||'—']
    ];
    m.querySelector('[data-hv-sub]').textContent=info.type==='repairs'?'Repair / Invoice record':'Device trade record';
    m.querySelector('[data-hv-grid]').innerHTML=fields.map(([a,b])=>`<div class="gf-hv-item ${a==='Issue / Details'?'gf-hv-full':''}"><span class="gf-hv-label">${esc(a)}</span><span class="gf-hv-value">${esc(b)}</span></div>`).join('');
  ...
  function add() {
    injectStyles();
    const cards=[...document.querySelectorAll('div.rounded-2xl')].filter(el=>{
      if(el.dataset.gfHistoryPro==='1')return false;
      const text=(el.innerText||'').trim();
      return text && /\b(?:GF|ACC|SALE|DVB)-[A-Za-z0-9-]+/.test(text) && el.querySelector('span');
    });
    cards.forEach(card=>{
      const text=(card.innerText||'').replace(/\s+/g,' ');
      const match=text.match(/\b(?:GF|ACC|SALE|DVB)-[A-Za-z0-9-]+/);
      if(!match)return;
  ...
  setTimeout(add,500);setTimeout(add,1500);setInterval(add,2500);
})();
...
// Adds multiple issue rows, discount calculation and persists the full bill breakdown.
...
function installJobSheetEnhancer() {
  ...
  card.innerHTML = `
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-700/70">
      <div class="rounded-xl border border-slate-700 bg-slate-950/40 p-3"><div class="text-[11px] uppercase tracking-wide font-black text-slate-500">Subtotal</div><div data-gf-subtotal class="text-lg font-black text-white mt-1">NPR 0</div></div>
      <div class="rounded-xl border border-slate-700 bg-slate-950/40 p-3"><div class="flex items-center justify-between gap-2"><div class="text-[11px] uppercase tracking-wide font-black text-slate-500">Discount</div><select data-gf-discount-type class="bg-transparent border border-slate-700 rounded-lg px-2 py-1 text-xs font-bold text-slate-200 outline-none"><option value="amount">NPR</option><option value="percent">%</option></select></div><input data-gf-discount-value type="number" min="0" step="0.01" value="0" placeholder="Discount" class="w-full mt-2 p-2.5 rounded-xl bg-slate-950 border border-slate-700 text-white text-sm outline-none" /></div>
      <div class="rounded-xl border border-blue-500/30 bg-blue-500/10 p-3"><div class="text-[11px] uppercase tracking-wide font-black text-blue-300">Grand Total</div><div data-gf-grand-total class="text-xl font-black text-blue-300 mt-1">NPR 0</div></div>
    </div>
  `;

  const photoBlock = [...form.children].find(el => (el.textContent || '').includes('Customer Photo (Optional)'));
  const submit = form.querySelector('button[type="submit"]');
  if (photoBlock) form.insertBefore(card, photoBlock); else if (submit) form.insertBefore(card, submit); else form.appendChild(card);
  ...
  const render = () => {
    ...
    state.issues.forEach((issue, index) => {
      ...
      row.innerHTML = `<select data-issue-preset class="md:col-span-5 p-3 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white outline-none"><option value="">Select issue from list...</option>${ISSUE_PRESETS.map(p => `<option value="${p.name.replace(/"/g, '&quot;')}">${p.name}</option>`).join('')}<option value="__manual__">✎ Manual Issue</option></select><input data-issue-name type="text" placeholder="Manual issue / service name" value="${String(issue.name || '').replace(/"/g, '&quot;')}" class="md:col-span-4 p-3 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white outline-none" /><input data-issue-amount type="number" min="0" step="0.01" placeholder="Amount (NPR)" value="${issue.amount ?? ''}" class="md:col-span-2 p-3 bg-slate-950 border border-slate-700 rounded-xl text-sm text-white outline-none" /><button type="button" data-remove-issue class="md:col-span-1 p-3 rounded-xl border border-rose-500/30 text-rose-400 hover:bg-rose-500/10 flex items-center justify-center" title="Remove issue">×</button>`;
      const preset = row.querySelector('[data-issue-preset]'); const name = row.querySelector('[data-issue-name]'); const amount = row.querySelector('[data-issue-amount]');
      preset.value = ISSUE_PRESETS.some(p => p.name === issue.name) ? issue.name : (issue.name ? '__manual__' : ''); name.value = issue.name || ''; name.style.display = preset.value === '__manual__' || !preset.value ? '' : 'none';
      preset.addEventListener('change', () => { if (preset.value === '__manual__' || !preset.value) { name.style.display = ''; if (preset.value === '__manual__') name.focus(); } else { name.style.display = 'none'; state.issues[index].name = preset.value; } calculate(); });
      ...
      row.querySelector('[data-remove-issue]').addEventListener('click', () => { if (state.issues.length === 1) state.issues[0] = { name: '', amount: '' }; else state.issues.splice(index, 1); render(); calculate(); });
  ...
  const calculate = () => {
    ...
    subtotalEl.textContent = `NPR ${subtotal.toLocaleString()}`; grandEl.textContent = `NPR ${grandTotal.toLocaleString()}`;
    const inputs = [...form.querySelectorAll('input')];
    const totalInput = inputs.find(i => i.placeholder === 'Total Cost (NPR)'); const issueInput = inputs.find(i => i.placeholder === 'Issue / Details (Optional)');
    if (totalInput) setReactInputValue(totalInput, grandTotal); if (issueInput) setReactInputValue(issueInput, issueSummary(state.issues) || '');
  };

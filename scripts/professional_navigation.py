...
nav = '''      {/* Premium Top Navigation — stable brand + overlay navigation */}
                  ...
                  return (
                    <div key={group.title} className="relative">
                      <button
                        ...
                        className={`w-full h-10 flex items-center justify-between gap-2 px-3 rounded-xl text-left transition-all ${hasActive ? 'bg-blue-600/10 text-blue-400' : t.textMuted} hover:bg-blue-600/10`}
                      >
                        <span className="text-xs sm:text-sm font-black tracking-wide truncate">{group.title}</span>
...
print('Stable professional navigation applied.')
